    ...
    <div id='currencyItem' style={style}>
      <div className='flex gap-2 h-42 items-center'>
        <div className='flex w-full justify-between items-center gap-2'>
          ...
          <a href={isBase ? undefined : `/chart?q=${(baseCur + '-' + cur).toUpperCase()}`}
            className="text-start tooltip flex items-center gap-2 h-[42px] w-[300px]" data-tip={name ?? ''}>
            <CountryImg code={cur} />
            {cur.toUpperCase()}
          </a>
          ...
          {isBase
            ...
            />
            : <div onClick={() => onSelectBase(cur)} className='w-[240px] text-end'>
              {val2Show}
            </div>}
